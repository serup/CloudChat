...
public:
  ...
  void Set(const char* String) {
    MD5Context Context;
    Init(&Context);
    Update(&Context, (_BYTE*)String, strlen(String));
    Final(&Context);
    CreateString();
  }
         bool IsValid() const {return *sMD5!=0;}
  const char* GetMD5s() const {return  sMD5;}
  const _BYTE* GetMD5n() const {return Digest;}
...
// Continues an MD5 message-digest operation, processing another message block, and updating the Context.
  void Update(MD5Context *Context, unsigned char *input, DWORD inputLen) {
    ...
    index=(unsigned int)((Context->count[0] >> 3) & 0x3F); // Compute number of _BYTEs mod 64
...
// F, G, H and I are basic MD5 functions.
  DWORD _F(DWORD xx, DWORD y, DWORD z) {return (xx & y) | (~xx &  z);}
...
// Rotation is separate from addition to prevent re-computation.
  void FF(DWORD& a, DWORD b, DWORD c, DWORD d, DWORD x, DWORD s, DWORD ac) {a+=_F(b,c,d)+x+ac; RotateLeft(a,s); a+=b;}
